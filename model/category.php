...
class Cate extends DB implements Icate
{
    ...
    function getCatesByProductId($masp)
    {
        $stm = $this->db->prepare(
            "SELECT c.tendm FROM category c INNER JOIN product p ON p.madm= c.madm WHERE p.masp=$masp ");
        $stm->execute();
        return $stm->fetchAll();
    }
    ...
    {
        $rows = $this->db->query("SELECT * FROM " . self::tableName . " WHERE madm= $madm");
        foreach ($rows as $r) {
            $row  = $r;
        }
        return $r;
